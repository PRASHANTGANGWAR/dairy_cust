...
/**
 * Generated class for the PreviousDeliveryPage page.
 *
...
@IonicPage()
@Component({
  selector: 'page-previous-delivery',
  templateUrl: 'previous-delivery.html',
})
...
previousDeliverys: any=[]=[];
 ...
 public customerId : number = 98452;
  ...
  AppUsers: any=[];
  Products: any=[];
  ProductsQuantity: any=[]; // array to save data from server to store quantity of 3 products
  pendingProductsQuantity:any=[];// array to save data from server to store quantity of 3 products which is pending and to be delivered coming day
  status: any=[];
 ...
 public isProductExist(id: any) {
  ...
  updateSchedule() {debugger;
    this.db.getpending().then(data => {
       this.AppUsers = [];
       console.log(data.length+"1");
       debugger;
       for(var z=0;z<data.length;z++){
            this.AppUsers.push(JSON.parse(data[z].jsondata));
        this.AppUsers=window.localStorage.setItem('previous_deliveries',(data[z].jsondata));
